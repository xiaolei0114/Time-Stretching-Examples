...
WavFile::WavFile(string tmpPath)
{
    position = 0;
    speed = 1.0;
    soundStatus = NONE;
...
void WavFile::setPath(string tmpPath)
{
    myPath = tmpPath;
    
}

void WavFile::setLooping(bool loop)
{
    if(loop) {
        soundStatus |= LOOPING;
    }
...
vector<std::pair<float, float> > WavFile::getPausePostionPairs()
{
    ...
    float minPauseTime = ((getNumChannels() == 1)? 0.05 : 0.15 )/ (getTotalTime());
    ...
    while ((long)position < getLength()) {
        if(myChannels == 1) {
            ...
            if (abs(sampleL) < 5) { // 5 or 10
                ...
                if (mutePositions.size() < minPauseTime * getLength() / getNumChannels()) {
...
int WavFile::getTotalTime()
{
    return getLength()/getNumChannels()/getSampleRate();
}

//Get this !
void WavFile::drawWaveForm(int _x, int _y, int _w, int _h, vector<MiniMaxima> * _waveForm)
{
...
void WavFile::setPaused(bool bPaused)
{
    if(bPaused) {
        soundStatus |= PAUSED;
    }
...
void WavFile::setSpeed(double spd)
{
    speed = spd;
}

double WavFile::getSpeed()
{
    return speed;
}
...
int WavFile::getDataSize()
{
    return myDataSize;
}

double WavFile::update()
{
...
// write out the wav file with the data in this class (Can be used for trim the Wav)
bool WavFile::save()
{
    ofToDataPath(myPath);
