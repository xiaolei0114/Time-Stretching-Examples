...
#pragma mark - newly added, override

- (NSTimeInterval)fileDuration
{
	return _time * [super fileDuration];
}


- (double)currentPosition
{
    return  [self currentTime] / ([self fileDuration]);
}

- (NSTimeInterval)currentTime
{
    return (NSTimeInterval)mTotalFramesPlayed * _time/ mSampleRate;
}

- (void)setCurrentPosition:(double)position
{
    NSTimeInterval time = position * [self fileDuration];
    [self setCurrentTime:time];
}

- (void)setCurrentTime:(NSTimeInterval)time
{
    [super setCurrentTime:time / _time];
    
    mTotalFramesPlayed = (time / [self fileDuration]) * (mTotalFramesInFile);
}
